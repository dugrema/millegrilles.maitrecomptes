...
from server_maitrecomptes.WebServerMaitreComptes import WebServerMaitreComptes
from server_maitrecomptes.Commandes import CommandMaitreComptesHandler

logger = logging.getLogger(__name__)

LOGGING_NAMES = ['server_maitrecomptes']
...
class MaitreComptesAppMain(WebAppMain):
    ...
    def init_command_handler(self) -> CommandMaitreComptesHandler:
        return CommandMaitreComptesHandler(self)

    async def configurer(self):
        await super().configurer()

    async def configurer_web_server(self):
        self._web_server = WebServerMaitreComptes(self.etat, self._commandes_handler)
...
async def demarrer():
    ...
    logger.info("Run main maitre des comptes")
    await main_inst.run()
    logger.info("Fin main maitre des comptes")
